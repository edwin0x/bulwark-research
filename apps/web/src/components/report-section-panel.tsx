	...
	return (
		<div className="bp-card p-6 md:p-8">
			...
			<div className="flex items-center gap-3 mb-6">
				...
				<h3 className="font-display text-base font-700 uppercase tracking-wider">{section.title}</h3>
			</div>

			{/* Highlights */}
			{section.highlights && section.highlights.length > 0 && (
				<div className="grid grid-cols-3 gap-4 mb-6 p-4 bg-surface border border-border">
					{section.highlights.map((h) => (
						<div key={h.label}>
							<div className="font-display text-xl font-bold">{h.value}</div>
							<div className="font-mono text-[9px] text-dim uppercase tracking-[0.08em]">
								{h.label}
							</div>
						</div>
					))}
			...
			<p className="text-ink leading-relaxed text-sm">{section.previewParagraph}</p>
