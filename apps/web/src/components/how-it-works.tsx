        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-4">
          {steps.map((step, i) => (
            <div key={step.num} className={`relative reveal delay-${i + 2}`}>
              ...
              {i < 2 && (
                <div className="hidden md:block absolute top-10 left-[calc(50%+40px)] w-[calc(100%-80px)] h-px bg-gradient-to-r from-ink-border to-ink-border/0"></div>
              )}
              ...
              <div className="text-center md:text-left">
                ...
                <div className="inline-flex items-center gap-3 mb-6">
                  ...
                  <span className="w-8 h-px bg-ink-border"></span>
                ...
                <p className="text-muted text-sm leading-relaxed max-w-xs mx-auto md:mx-0">{step.desc}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="mt-20 reveal delay-4">
          <div className="card-glass rounded-2xl p-6 md:p-8 max-w-4xl mx-auto">
            ...
            <div className="flex items-center gap-2 mb-2">
              <div className="w-3 h-3 rounded-full bg-vermillion/60"></div>
              <div className="w-3 h-3 rounded-full bg-amber/60"></div>
              <div className="w-3 h-3 rounded-full bg-emerald/60"></div>
              <span className="ml-3 font-mono text-xs text-dim">bulwark validate --deep --internet-scale</span>
            </div>
            ...
            <div className="flex items-center justify-between py-3 mb-4 border-b border-ink-border/50">
              <div className="flex items-center gap-4">
                ...
                <span className="font-mono text-[10px] text-dim hidden md:inline">3.2M tokens processed</span>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-2">
              {swarmAgents.map((agent, i) => (
                <div key={agent.name} className="flex items-center gap-3 py-1.5">
                  <div className="w-1.5 h-1.5 rounded-full animate-pulse-dot" style={{ background: agent.hex, animationDelay: `${i * 0.2}s` }}></div>
                  <span className="font-mono text-[11px] text-muted w-40 shrink-0">{agent.name}</span>
                  ...
                  <span className="font-mono text-[10px] text-dim w-10 text-right">{agent.pct}%</span>
                </div>
              ))}
            </div>
            ...
            <div className="mt-4 pt-4 border-t border-ink-border/50 space-y-1.5">
              <div className="font-mono text-[10px] text-dim opacity-70">
                <span className="text-emerald">✓</span> Market Sizing: Ingested 23 industry reports, 4 government databases
              </div>
              <div className="font-mono text-[10px] text-dim opacity-60">
                <span className="text-amber">→</span> Competitor Intel: Crawling 156 company profiles, analyzing funding rounds...
              </div>
              <div className="font-mono text-[10px] text-dim opacity-50">
                <span className="text-cyan">→</span> Financial Model: Running Monte Carlo simulation (10,000 iterations)...
              </div>
