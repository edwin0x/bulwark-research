		...
		<section id="what-you-get" className="relative py-24 px-6">
			...
			<div className="max-w-6xl mx-auto">
				<div className="text-center mb-16">
					<div className="overline-divider max-w-sm mx-auto mb-4 reveal">
				...
				<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
					{agents.map((agent, i) => (
						<div
							key={agent.name}
							className={`card-glass rounded-2xl p-6 reveal delay-${(i % 4) + 2} ${i < 2 ? 'sm:col-span-1 lg:col-span-2' : ''}`}
						>
							<div className="flex items-start justify-between mb-4">
								<span className="text-2xl" style={{ color: agent.hex }}>
									{agent.icon}
								</span>
								<span className="font-mono text-[10px] text-dim tracking-widest uppercase">
									Swarm Layer {String(i + 1).padStart(2, '0')}
								</span>
							</div>
