...
export function ReportPreview() {
  return (
    <section className="relative py-24 px-6 overflow-hidden">
      ...
      {/* Background glow */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-vermillion/5 rounded-full blur-[120px] pointer-events-none"></div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          ...
          <div>
            ...
            <p className="text-muted mt-4 leading-relaxed reveal delay-2">
              Not a ChatGPT summary. A structured, data-backed intelligence brief synthesized from thousands of data points across hundreds of sources — delivered to your inbox in 30 minutes. The kind Big 4 firms charge $150K+ for.
            </p>

            <ul className="mt-8 space-y-4">
              {bulletItems.map((item, i) => (
                <li key={i} className={`flex items-start gap-3 reveal delay-${i + 3}`}>
                  <span className="mt-1 w-1.5 h-1.5 rounded-full bg-vermillion shrink-0"></span>
          ...
          <div className="reveal delay-3">
            <div className="report-glow rounded-2xl border border-ink-border bg-ink-light/80 p-6 md:p-8">
              ...
              <div className="flex items-center justify-between mb-6">
                <div>
                  <div className="font-mono text-[10px] text-dim tracking-widest uppercase mb-1">Bulwark Report · Confidential</div>
              ...
              <div className="h-px bg-ink-border mb-6"></div>
                    ...
                    {competitors.map((comp, i) => (
                      <div key={comp} className="flex items-center gap-3">
                        ...
                        <div className="flex gap-0.5">
                          {[1, 2, 3, 4, 5].map((_, j) => (
                            <div key={j} className={`w-4 h-1.5 rounded-sm ${j <= 3 - i ? 'bg-amber/60' : 'bg-ink-border/40'}`}></div>
                          ))}
                        </div>
                      </div>
                    ))}
                  ...
                  <div className="flex items-center gap-4">
                    {risks.map((risk) => (
                      <div key={risk.label} className="flex items-center gap-2">
                        <div className="w-1.5 h-1.5 rounded-full" style={{ background: risk.hex }}></div>
                        <span className="font-mono text-[10px] text-dim">{risk.label}: <span style={{ color: risk.hex }}>{risk.level}</span></span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className="mt-6 flex items-center justify-between">
                <span className="font-mono text-[9px] text-dim tracking-widest uppercase">Generated Feb 17, 2026 · 32 pages · 1,247 sources · 47 agents</span>
                <div className="flex gap-2">
                  <div className="px-2 py-1 rounded text-[10px] font-mono text-dim border border-ink-border hover:border-muted cursor-pointer transition-colors">PDF</div>
                  <div className="px-2 py-1 rounded text-[10px] font-mono text-dim border border-ink-border hover:border-muted cursor-pointer transition-colors">Share</div>
                </div>
              </div>
            </div>
          </div>
